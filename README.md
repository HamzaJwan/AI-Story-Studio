...
8. خط زمني لحالة كل مشهد (مع سجل العمليات الأخيرة)، مكتبة أصول لكل ملفات المشروع مع معاينة مباشرة (صوت/صورة/فيديو)، ولوحة مراجعة جودة (اعتماد/إعادة/رفض + تصفية) قبل التصدير النهائي.
9. سؤال بسيط عن مشروعك الحالي عبر Ollama مباشرة ("المساعد المحلي") — بدون RAG حقيقي أو بحث إنترنت.
10. تحميل حزمة المشروع كاملة (`export.zip`).

**ما لا يفعله بعد:** لا فيديو بالذكاء الاصطناعي (motion حقيقي)، لا قاعدة بيانات/مستخدمين متعددين، لا مساعد محادثة محلي كامل (RAG/بحث/ذاكرة محادثة — Open WebUI يبقى الخطة المستقبلية لذلك). هذه مخططة لاحقاً في [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
3. تابع الخطوات بالترتيب من شريط "Studio Workflow" أعلى الصفحة: المشاهد ← الصوت ← الصور ← الفيديو والترجمة ← الخط الزمني ← مكتبة الأصول ← مراجعة الجودة ← استوديو الصور المستقل ← المساعد المحلي ← التصدير.
